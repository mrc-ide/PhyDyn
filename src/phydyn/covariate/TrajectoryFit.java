...
public abstract class TrajectoryFit extends Distribution {
	...
	public Input<PopModelODE> popModelInput = new Input<>("popmodel","Population Model",Validate.REQUIRED);
	public Input<String> dataInput = new Input<>("data", "Data entered in table format",Validate.REQUIRED);
	public Input<Boolean> headerInput = new Input<>("header", "Flag indicating if headers are included in data/table", true);
	...
	// todo: id collection (subset of headers) as input
	public double[][] getDataAsMatrix() {
		double[][] m = new double[numrows][numcols];
		for (int col=0; col < numcols; col++) {
			double[] colvals = this.getColumnAsDouble(col);
			for(int row=0; row < numrows; row++)
				m[row][col] = colvals[row];
		}
		return m;
	}
