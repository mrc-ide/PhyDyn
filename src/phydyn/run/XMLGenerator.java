...
public class XMLGenerator extends Runnable {
	public Input<String> xmlTypeInput = new Input<>("xmlType", "XML Phydyn files: likelihood, ParamMCMC", Validate.REQUIRED);
	...
	public Input<Boolean> createTypeTraitInput = new Input<>("createTypeTrait","Date tips with Date trait (using t1)",new Boolean(false));
	...
	private String xmlFile, xmlType, newick;
	...
	final static String LIKELIHOOD = "likelihood", PARAM = "paramMCMC", TREE = "treeMCMC";
	...
	public void initAndValidate() {
		xmlType = xmlTypeInput.get();
		if (xmlType.equals(LIKELIHOOD)) {
			System.out.println("Generating Likelihood test xml");
		} else if (xmlType.equals(PARAM)) {
			System.out.println("Generating parameter sampling xml");
			throw new IllegalArgumentException("xmlType "+xmlType+" not implemented");
		} else if (xmlType.equals(TREE)) {
			System.out.println("Generating tree sampling xml");
			throw new IllegalArgumentException("xmlType "+xmlType+" not implemented");
		} else {
			throw new IllegalArgumentException("Incorrect xmlType value. Use "+LIKELIHOOD+"/"+PARAM+"/"+TREE+"\n");
		}
	...
	void writeHead(FileWriter writer) throws IOException {
		writer.append("<?xml version=\"1.0\"?>\n");
		writer.append("<beast version=\"2.5\" namespace=\"");
		writer.append("phydyn.run:phydyn.model:phydyn.distribution:");
		writer.append("beast.util:beast.evolution.alignment:beast.evolution.tree");
		writer.append("\">\n\n");
		
	}
	
	void writeTaxa(FileWriter writer) throws IOException {
		writer.append("<taxa spec=\"TaxonSet\" id=\""+ taxaID +"\">");
		int three=3;
		for(int i = 0; i < numTips; i++) {
			...
			writer.append("<taxon spec=\"Taxon\" id=\""+ tipNames[i] +"\" /> ");
			three++;
		}		
	     writer.append("\n</taxa>\n");
	...
	void writeDateTrait(FileWriter writer) throws IOException {
		//<traits spec="TraitSet" id="simpleDateTrait" traitname="date" taxa="@simpletaxa">
		writer.append("<traits spec=\"TraitSet\" id=\""+dateTraitID+"\" traitname=\"date\" taxa=\"@"+taxaID+"\">");		
		...
		for(int i = 1; i < numTips; i++) {
			...
			writer.append(","+tipNames[i]+"="+(mrTipDate-tipNodes[i].getHeight()));
			ten++;
		}	
		//</traits>
		writer.append("\n</traits>\n");
	...
	void writeTypeTrait(FileWriter writer) throws IOException {
		//<traits spec="TraitSet" id="--" taxa="--">
		writer.append("<traits spec=\"TraitSet\" id='"+typeTraitID);
		writer.append("' traitname='Types' taxa=\"@"+taxaID+"\">");		
		// <taxon>=<deme>,
		// first taxon
		...
		for(int i = 1; i < numTips; i++) {
			...
			writer.append(","+tipNames[i]+"="+stateName);
			ten++;
		}	
		//</traits>
		writer.append("\n</traits>\n");
	...
	void writeTree(FileWriter writer) throws IOException {
		treeID = modelName+"-tree";
		// <tree spec="TreeParser" id="simpletree" adjustTipHeights="false" IsLabelledNewick="true">
		writer.append("<tree spec=\"TreeParser\" id=\""+treeID+"\" ");
		writer.append("adjustTipHeights=\"false\" IsLabelledNewick=\"true\">\n");
		// <input name="newick">
		writer.append(" <input name=\"newick\">\n");
		writer.append("  "+newick+"\n");
	    // </input>
		writer.append("  </input>\n");
		//  <trait idref="simpleDateTrait"/>
		if (createDateTrait) {
			writer.append("  <trait idref='"+dateTraitID+"'/>\n");
		}
		// if date trait
	    //<trait idref="simpleDateTrait"/>
		writer.append("</tree>\n");
	...
	void writeLikelihood(FileWriter writer) throws IOException {
		stlhID = "stlh";
		String intervalsID = modelName+"-intervals";
		//<distribution spec="STreeLikelihoodODE" id="stlh"
		writer.append("<distribution spec=\"STreeLikelihoodODE\" id=\""+stlhID+"\"\n");
		//		popmodel='@twodeme'  
		writer.append(" popmodel='@"+modelName+"'");
		writer.append(" useStateName='true'");
		if (createTypeTrait) {
			writer.append(" typeTrait='@"+typeTraitID+"'");
		}
		writer.append(" equations='PL1' \n");
		writer.append("  forgiveY='true' stepSize='0.001' minP='.001'");
		...
		writer.append("  <treeIntervals spec=\"STreeIntervals\" tree=\"@"+treeID+"\" id=\""+intervalsID+"\"/>\n");
		//	</distribution>
		writer.append("</distribution>\n");
	}
	

	void writeTail(FileWriter writer) throws IOException {
