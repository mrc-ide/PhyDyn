# run: R -f genTrees.R --args <modelname> <numTrees>
# R -f genTrees.R --args hiv2model 2  > log.txt

# Description: Script that generates <numTrees> Newick files using
#              MASTER simulations

# Arguments:
# <modelName>.xml must exist: MASTER model
...
require(rjson)
#require(XML)
#source("toolsMaster.R")

# Converts a MASTER generated Nexus file in Newick format.
...
processNexus <- function(nexusFile, newickFile, numTips) {
  ...
  # Replace statename with _statename
  #for(stateName in stateNames) {
  #  print(paste("replacing:",stateName))
  #  nwk <- gsub(stateName,paste("_",stateName,sep=''),nwk)
  #}
...
showFromJSON <- function(jsonFile) {
  ...
  # Comparing to deterministic model - output ode example
  # o <- ode(y0, times, dY)
...
if (length(clargs) < 2) {
   stop("Must enter model name (no suffix) and number of trees")
}
modelName <- clargs[1]
numTrees <- strtoi(clargs[2])
if (is.na(numTrees)) {
  stop(paste('Number of trees should be an integer:',clargs[2]))
}
